...
Carguemos también `DT` para tener unas tablas bonitas:
```{r loadDT}
library(DT)
```
...
```{r promedioMediana}
irisDT[, .(promedio = mean(Sepal.Length), mediana = median(Sepal.Length)), by = Species]
```

El `.()` que encierra las expresiones en _j_ son un nombre alternativo para la función `list()` (ensaya cambiando el punto inicial por `list`), y sirven para que data.table nos devuelva un resultado de clase `data.table`.

#### Expresiones

La _j_ puede tomar no sólo funciones, sino también **expresiones**. Esto es útil en algunos contextos. Por ahora no mucho, así que voy a mostrar un par de ejemplos medio tontos de lo que esto significa. En algún blog posterior espero mostrar algo más relevante que se puede lograr con esto.
